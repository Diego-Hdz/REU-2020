...
public class ThemisInterJobScheduler extends InterJobScheduler {
	...
	public List<Bid> pickWinningBids(List<GPU> gpu_set, List<Bid> bid_set) {
		...
		try {
			...
			/*
			 * for (IntraJobScheduler job: bidsPerJob.keySet()) { double logOldBenefit =
			 * 1000; if (!Double.isInfinite(job.getOldBenefit())) { logOldBenefit =
			 * Math.log(job.getOldBenefit()); // log(T_s_old/T_i_old) }
			...
			if (solver.get(GRB.IntAttr.Status) == GRB.Status.OPTIMAL || solver.get(GRB.IntAttr.Status) == GRB.Status.TIME_LIMIT) {
				...
				// double maxValuationAbsentJob = pickWinningBidsTruthTelling(gpu_set, bid_set,
				// job);
	...
	public void onResourceAvailable(List<GPU> gpu_set) {
		...
		while (gpu_set.size() > 0) {
			...
			//need to now check which jobs to consider for bidding
			...
			// These are jobs need to definitely consider for bidding.
			List<JobFairness> fairnessValuesDefinite = new ArrayList<>();
			// These are jobs that are tied at the lowest Ts/Ti. Randomly pick jobs
			// from this list.
