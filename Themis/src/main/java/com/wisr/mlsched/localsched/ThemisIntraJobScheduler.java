...
/**
		...
		*/
		...
		for (Integer gpuSize : gpuBenefit.keySet()) {
			...
			if (bidsForGPUSize != null) {
				...
				bids.addAll(bidsForGPUSize.stream().filter(p -> p.mNewPscore == largestPlacementScore)
	...
	private Bid prepareBidWithGPUs(String bitmask, List<GPU> offeredGPUs) {
		...
		/*
		 * if(Double.compare(ratio, 1) < 0) { // no point in making bid return null; }
		 */
		if (Double.compare(newSpeedup, 1) < 0) {
			return null;
		}

		double oldSpeedup = getGPUsAvailableForNextIteration().size()
